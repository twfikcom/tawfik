...
interface DomainsPageProps {
  onBack: () => void;
  onDomainInquire: (domainName: string) => void;
}

const DomainsPage: React.FC<DomainsPageProps> = ({ onBack, onDomainInquire }) => {
    ...
    <div className="animate-fade-in pt-32 pb-20">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <AnimateOnScroll className="text-center mb-16">
          <h1 className="text-4xl md:text-5xl font-extrabold font-heading uppercase">{t('domains.title')}</h1>
          <p className="text-lg text-gray-600 dark:text-gray-400 mt-4 max-w-2xl mx-auto">{t('domains.subtitle')}</p>
        </AnimateOnScroll>
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {DOMAINS.map((domain, index) => (
            <DomainListing key={domain.id} domain={domain} onInquire={onDomainInquire} index={index} />
          ))}
        </div>
      </div>
    </div>
